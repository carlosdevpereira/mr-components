...
class HttpRequest {
	...
	setFields(fields = []) {
		if (!fields || fields.length === 0)
			throw new Error(
				'Fields array is a required parameter for setFields and must have at least one item'
			)
	...
	whereHigherOrEqual(fieldName, value) {
		if (!fieldName)
			throw new Error('Field name is a required parameter for whereHigherOrEqual clause')
	...
	whereLowerOrEqual(fieldName, value) {
		if (!fieldName)
			throw new Error('Field name is a required parameter for whereLowerOrEqual clause')
	...
	whereContains(fieldName, value, caseSensitive = true) {
		if (!fieldName)
			throw new Error('Field name is a required parameter for whereContains clause')
	...
	whereStartsWith(fieldName, value, caseSensitive = true) {
		if (!fieldName)
			throw new Error('Field name is a required parameter for whereStartsWith clause')
	...
	whereEndsWith(fieldName, value, caseSensitive = true) {
		if (!fieldName)
			throw new Error('Field name is a required parameter for whereEndsWith clause')
	...
	setAuthorization(token) {
		if (!token)
			throw new Error('Authorization token is a required parameter for setAuthorization')
	...
	send() {
		...
		if (this.method === 'get' && this.type === 'retrieve') {
			...
			if (this.sortBy.field)
				url += `&[order_by]=${this.sortBy.field} ${this.sortBy.direction}`
